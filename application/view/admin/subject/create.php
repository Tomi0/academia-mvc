...
<div class="panel panel-green">
    ...
    <div class="panel-body">
        ...
        <form action="/adminsubjects/store" method="POST">

            <div class="row">
                <div class="form-group col-lg-6 <?= isset($errors['name']) && $errors['name'] !== true ? 'has-error' : '' ?>">
                    <label for="name">Nombre de la asignatura</label>
                    <input type="text" name="name" id="name" placeholder="introduzca un nombre" class="form-control" value="<?= isset($_POST['name']) ? $_POST['name'] : '' ?>">
                ...
                <div class="form-group col-lg-6 <?= isset($errors['category_id']) && $errors['category_id'] !== true ? 'has-error' : '' ?>">
                    ...
                    <select name="category_id" id="category_id" class="form-control">
                        <?php foreach($categories as $category) { ?>
                            <option <?php if (isset($_POST['category_id'])) { echo $_POST['category_id'] == $category->id ? 'selected' : ''; } ?> value="<?= $category->id ?>"><?= $category->name ?></option>
                        <?php } ?>
                    </select>
                    ...
                    <select name="user_id" id="user_id" class="form-control">
                        <?php foreach($users as $user) { ?>
                            <?php if ($user->rol->name == 'profesor') { ?>
                                <option <?php if (isset($_POST['user_id'])) { echo $_POST['user_id'] == $user->id ? 'selected' : ''; } ?> value="<?= $user->id ?>"><?= $user->name ?></option>
                            <?php } ?>
                        <?php } ?>
                    </select>
            ...
            <div class="row">
                <div class="form-group">
                    <button type="submit" class="btn btn-primary margen-izquierda-normal">Crear</button>
                </div>
            </div>
